...
public class StaffEvaluationQuestionPageController implements StaffEvaluationController {
    ...
    public String questions(@PathVariable Long id,
                            ...
                            @PageableDefault(page = DEFAULT_PAGE, sort = UUID, direction = ASC)
        ...
        var staffEvaluationQuestions = staffEvaluationQuestionService.findAll(assignedFilter, pageableOf(assignedPageable));
        addPageAttributes(model, ASSIGNED_PAGE, assignedPageable, staffEvaluationQuestions);
        ...
        model.addAttribute(ASSIGNED_QUESTIONS, staffEvaluationQuestions);
    ...
    @DeleteMapping("/staff-evaluations/{id}/questions")
    public String removeFilteredQuestions(@PathVariable Long id,
                                          @RequestParam(required = false) String search,
                                          @RequestParam(required = false, name = R_PROJECT_ROLE) Long projectRole,
                                          @RequestParam(required = false) Long skill) {
        var filter = QuestionFilter.builder()
        ...
        staffEvaluationService.removeQuestions(id, filter);
        return redirectToQuestions(id);
    }
    ...
    private String buildAssignedQuestionQuery(StaffEvaluationQuestionFilter filter) {
        ...
        filter.appendQueryParam(query, R_PROJECT_ROLE, filter.projectRole());
