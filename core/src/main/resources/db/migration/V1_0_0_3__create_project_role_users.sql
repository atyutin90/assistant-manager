...
INSERT INTO user_responsible(user_id, responsible_id)
SELECT users.id, responsible.id
FROM
    (VALUES  ('analyst1', 'analyst2'),
             ('backend1', 'backend2'),
             ('devops1', 'devops2'),
             ('frontend1', 'frontend2'),
             ('autoqa1', 'autoqa2'),
             ('qa1', 'qa2')) AS data(username, responsible_username), users, users as responsible
WHERE data.username = users.username AND data.responsible_username = responsible.username;
